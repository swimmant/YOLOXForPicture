...
def detect_cap(frame):# this function refers to occlusion det
    img=frame
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    cap = cv2.VideoCapture(args.path if args.demo == "=" else args.camid)
    width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)  # float
    height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)  # float
    fps = cap.get(cv2.CAP_PROP_FPS)
    if args.save_result:
        save_folder = os.path.join(
            './tempImage/', time.strftime("%Y_%m_%d_%H_%M_%S", current_time)
        )
        os.makedirs(save_folder, exist_ok=True)
        if args.demo == "video":
    ...
    while True:
        ...
        if ret_val:
            ...
            cell_phone_detected = predictor.visual(outputs[0], img_info, predictor.confthre) # this function refers to cell phone det
            cap_detected = detect_cap(frame) # this function refers to occlusion det
            ...
            if cap_detected:
                ...
                #保存检测图片
                save_folder = os.path.join(
                    './tempImage/', time.strftime("%Y_%m_%d", current_time)
                )
                os.makedirs(save_folder, exist_ok=True)
                save_file_name = os.path.join(save_folder, 'detectedPic%s.jpg' % datetime.datetime.now().strftime('%H_%M_%S'))
                logger.info("Saving detection result in {}".format(save_file_name))
                # cv2.imshow("fram",frame)
                # cv2.waitKey(0)

                # cv2.imshow("fram1",cell_phone_detected)
                # cv2.waitKey(0)
